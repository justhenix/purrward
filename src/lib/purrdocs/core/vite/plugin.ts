...
import type { PurrdocsConfig } from '../config/types.ts';
...
export interface PurrdocsPluginOptions {
	config: PurrdocsConfig;
	contentDir?: string;
}
...
/**
 * Purrdocs Vite plugin for virtual modules and optimization
 */
export function purrdocs(options: PurrdocsPluginOptions): Plugin {
	const contentDir = options.contentDir || 'src/content/docs';
	let root = '';
	let resolvedPurrdocsConfig: PurrdocsConfig;
	...
	return {
		...
		async configResolved(config) {
			...
			resolvedPurrdocsConfig = mergeConfig(options.config);
		...
		resolveId(id) {
			...
			if (id === 'purrdocs:config') {
				return 'virtual:purrdocs:config';
			}
			if (id === 'purrdocs:navigation') {
				return 'virtual:purrdocs:navigation';
			}
			if (id === 'purrdocs:search') {
				return 'virtual:purrdocs:search';
			}
			if (id === 'purrdocs:content') {
				return 'virtual:purrdocs:content';
			}
			return null;
		},

		async load(id) {
			// Virtual module: purrdocs:config
			if (id === 'virtual:purrdocs:config') {
				// Inline the resolved config object
				return `
					export const config = ${JSON.stringify(resolvedPurrdocsConfig, null, 2)};
					export default config;
				`;
			}

			// Virtual module: purrdocs:content
			if (id === 'virtual:purrdocs:content') {
				return await generateContentModule(root, contentDir);
			}

			// Virtual module: purrdocs:navigation
			if (id === 'virtual:purrdocs:navigation') {
				return `
					import { generateSidebar, getPrevNext, generateBreadcrumbs } from 'purrdocs-core';
			...
			// Virtual module: purrdocs:search
			if (id === 'virtual:purrdocs:search') {
	...
	content: ${JSON.stringify(rawContent)}
				...
				return `
import { buildSearchIndex } from 'purrdocs-core';
