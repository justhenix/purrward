...
import { initializeHighlighter } from './highlighter.ts';
...
// Direct plugin with lazy highlighter init for dev-server markdown transforms.
export const remarkCodeComponents = () => {
	return async (tree: ASTNode) => {
		...
		if (usedBlock || usedTabs) {
			const imports = `import { ${['CodeBlock', usedTabs ? 'CodeTabs' : null].filter(Boolean).join(', ')} } from 'purrdocs-core';`;
			if (tree.children) {
