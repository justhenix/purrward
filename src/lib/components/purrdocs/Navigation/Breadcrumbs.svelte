...
<script lang="ts">
	import type { Breadcrumb } from 'purrdocs-core';
	...
	const displayItems = $derived.by((): DisplayBreadcrumb[] => {
...
<style>
	...
	.link:hover {
		color: var(--purrdocs-color-primary, #2563eb);
		border-color: var(--purrdocs-color-primary, #2563eb);
	}
	...
	.separator {
		color: var(--purrdocs-color-border, #e5e7eb);
	}

	.ellipsis {
		color: var(--purrdocs-color-text-tertiary, #9ca3af);
	}
</style>
